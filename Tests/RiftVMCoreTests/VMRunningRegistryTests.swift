...
final class VMRunningRegistryTests: XCTestCase {
    private func makeRegistryPair() throws -> (VMRunningRegistry, VMRunningRegistry) {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("RiftVMRunLeaseTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testLeaseIsExclusiveAcrossEquivalentPaths() {
        ...
        let root = URL(fileURLWithPath: "/tmp/RiftVM Lease Test")

        let first = registry.acquire(rootPath: root)

        XCTAssertNotNil(first)
        let equivalent = URL(fileURLWithPath: "/tmp/../tmp/RiftVM Lease Test")
    ...
    func testOnlyOwnerCanTransitionAndReleaseLease() throws {
        let registry = VMRunningRegistry()
        let root = URL(fileURLWithPath: "/tmp/RiftVM Lease Owner Test")
    ...
    func testRepeatedShutdownRestartAndStaleCallbackSafety() throws {
        let registry = VMRunningRegistry()
        let root = URL(fileURLWithPath: "/tmp/RiftVM Repeated Restart Test")
    ...
    func testLeaseIsExclusiveAcrossRegistryInstances() throws {
        ...
        let root = URL(fileURLWithPath: "/tmp/RiftVM Cross Process Lease Test")
    ...
    func testDifferentMachinesCanRunConcurrently() throws {
        ...
        XCTAssertNotNil(firstRegistry.acquire(rootPath: URL(fileURLWithPath: "/tmp/RiftVM Concurrent A")))
        XCTAssertNotNil(secondRegistry.acquire(rootPath: URL(fileURLWithPath: "/tmp/RiftVM Concurrent B")))
    }

    func testRegistryDeinitReleasesKernelLease() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("RiftVMRunLeaseRecovery-\(UUID().uuidString)", isDirectory: true)
        let root = URL(fileURLWithPath: "/tmp/RiftVM Crash Recovery")
    ...
    func testRegistryAggregatesResourcesAcrossInstances() throws {
        ...
        let firstLease = try XCTUnwrap(first.acquire(rootPath: URL(fileURLWithPath: "/tmp/RiftVM Resource A")))
        ...
        let secondLease = try XCTUnwrap(second.acquire(rootPath: URL(fileURLWithPath: "/tmp/RiftVM Resource B")))
    ...
    func testResourceAdmissionFailsClosedWhenLockedMetadataIsCorrupt() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("RiftVMRunLeaseCorruption-\(UUID().uuidString)", isDirectory: true)
        let first = VMRunningRegistry(lockDirectory: directory)
        let second = VMRunningRegistry(lockDirectory: directory)
        let firstRoot = URL(fileURLWithPath: "/tmp/RiftVM Corrupt Resource A")
        ...
        let secondLease = try XCTUnwrap(second.acquire(rootPath: URL(fileURLWithPath: "/tmp/RiftVM Corrupt Resource B")))
