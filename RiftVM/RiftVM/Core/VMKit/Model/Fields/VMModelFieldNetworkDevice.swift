...
#if arch(arm64)
private let riftvmVMNetSuccess = vmnet_return_t.VMNET_SUCCESS
private let riftvmVMNetHostMode = vmnet_mode_t.VMNET_HOST_MODE
private let riftvmVMNetSharedMode = vmnet_mode_t.VMNET_SHARED_MODE
...
/// Prevents two RiftVM processes from independently reserving the same named
...
final class VMNetNamedNetworkOwnershipRegistry {
    ...
    init(directory: URL? = nil) {
        self.directory = directory ?? FileManager.default.urls(
            ...
            in: .userDomainMask
        )[0].appendingPathComponent("RiftVM/NetworkLeases", isDirectory: true)
    }
    ...
    func acquire(identifier: String, signature: String) -> VMNetNamedNetworkOwnershipResult {
        ...
        do {
            ...
            return .unavailable("RiftVM could not prepare its network ownership directory: \(error.localizedDescription)")
        }

        let descriptor = open(lockURL(identifier: identifier).path, O_RDWR | O_CREAT | O_CLOEXEC, S_IRUSR | S_IWUSR)
        guard descriptor >= 0 else {
            return .unavailable("RiftVM could not open the network ownership lock: \(String(cString: strerror(errno))).")
        }
        guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else {
            let lockError = errno
            close(descriptor)
            guard lockError == EWOULDBLOCK else {
                return .unavailable("RiftVM could not check network ownership: \(String(cString: strerror(lockError))).")
            }
        ...
        guard writeRecord(for: lease) else {
            ...
            return .unavailable("RiftVM could not record ownership of VMNet network ‘\(identifier)’.")
        }
...
struct VMModelFieldNetworkDevice: Codable, CustomStringConvertible {
        ...
        if let hostPortAvailability {
            for endpoint in externalEndpoints.keys.sorted() {
                ...
                case .unavailable(let detail):
                    return "RiftVM could not verify external \(transport.displayName) port \(port): \(detail). Choose another port or check local network permissions."
                }
            }
        }
        return nil
    ...
    func validationError(
        ...
        if !vmnetEntitlementGranted {
            return "The signed RiftVM app does not have the VMNet entitlement."
        }
    ...
    func createConfiguration() -> VMOSResult<VZNetworkDeviceConfiguration, String> {
        ...
        case .VMNetShared, .VMNetHost:
            ...
            var status = riftvmVMNetSuccess
            let mode = type == .VMNetShared ? riftvmVMNetSharedMode : riftvmVMNetHostMode
            ...
            if let externalInterface {
                ...
                guard result == riftvmVMNetSuccess else {
            ...
            if let networkIdentifier {
                ...
                case .alreadyOwned(let signatureMatches):
                    return .failure(
                        signatureMatches
                            ? "VMNet network ‘\(networkIdentifier)’ is active in another RiftVM process. Stop the virtual machine using it or close that RiftVM process, then try again."
                            : "VMNet network ‘\(networkIdentifier)’ is active in another RiftVM process with different settings. Choose another network name or close that RiftVM process."
                    )
