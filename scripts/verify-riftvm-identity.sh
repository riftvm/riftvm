...
shipping_paths=(CLI Casks GuestAgent RiftVM Tests Tools Experiments scripts .github Package.swift README.md THIRD_PARTY_NOTICES.md docs Assets Resources)
old_pattern='E[Z]VM|EasyVM|e[z]vm|easyvm'
...
path_matches="$(find "${shipping_paths[@]}" -path '*/.build' -prune -o \
  \( -name '*E[Z]VM*' -o -name '*EasyVM*' -o -name '*e[z]vm*' -o -name '*easyvm*' \) -print 2>/dev/null || true)"
