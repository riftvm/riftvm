    ...
    func onStepMovedIn(context: VMCreateStepperGuidePhaseContext) async -> VMOSResultVoid {
        ...
        let result = await creator.create(
            ...
            progress: { progressInfo in
                switch progressInfo {
                case .info(let log):
                    RiftVMLog.info(log, logger: RiftVMLog.lifecycle)
                    context.formData.addLog(log)
                case .error(let log):
                    RiftVMLog.error(log, logger: RiftVMLog.lifecycle)
        ...
        context.formData.creationCancellationKind = nil
        context.formData.canCancelCreation = false
        RiftVMLog.info("Virtual machine creation finished", logger: RiftVMLog.lifecycle)
        context.formData.isCreating = false
...
private final class VMPreinstalledImageDownloadCoordinator {
    ...
    func prepare(
        ...
        do {
            ...
            let stagingDirectory = FileManager.default.temporaryDirectory
                .appending(path: "RiftVM-manifest-\(UUID().uuidString)", directoryHint: .isDirectory)
            try FileManager.default.createDirectory(at: stagingDirectory, withIntermediateDirectories: true)
            defer { try? FileManager.default.removeItem(at: stagingDirectory) }
            let stagingManifestURL = stagingDirectory.appending(path: "riftvm-release-manifest.json")
            ...
            let manifestURL = cacheDirectory.appending(path: "riftvm-release-manifest.json")
    ...
    nonisolated private static func validateDistribution(_ manifest: PreinstalledImageManifest) throws {
        guard let archive = manifest.archive,
              archive.compression == "riftvm-sparse-stream+gzip",
    ...
    nonisolated private static func cacheDirectory(itemID: String, tag: String) throws -> URL {
        let directory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            .appending(path: "RiftVM/PreinstalledImages/\(itemID)/\(tag)", directoryHint: .isDirectory)
...
struct CreatePhaseCreatingView: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("RiftVM will download the selected image if needed, then create and install your virtual machine. You can leave this window open and wait once.")
                .font(.callout)
