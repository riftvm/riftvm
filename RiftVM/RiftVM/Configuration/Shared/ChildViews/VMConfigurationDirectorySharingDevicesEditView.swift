...
struct VMConfigurationDirectorySharingDevicesEditView: View {
    ...
    private static let linuxMountCommand =
        "sudo mkdir -p /mnt/riftvm && sudo mount -t virtiofs riftvm_shared /mnt/riftvm"
            ...
            Form {
                Section {
                    if configData.directorySharingDevices.isEmpty {
                        ContentUnavailableView(
                            "No Shared Folders",
                            systemImage: "folder",
                            description: Text("Choose a folder once. RiftVM configures the share automatically.")
                        )
                    } else {
                ...
                if configData.osType == .linux {
                    Section {
                        ...
                        Text("Run once after the guest starts. With one shared folder, its contents appear directly at /mnt/riftvm; with several, each appears as a named folder.")
                    }
                }
            }
            .formStyle(.grouped)
