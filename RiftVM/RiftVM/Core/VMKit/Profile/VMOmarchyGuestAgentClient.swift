...
public final class VMOmarchyGuestAgentClient {
    ...
    public func provisionOwner(_ request: VMOmarchyOwnerProvisioningRequest) async throws {
        ...
        guard result.success else {
            throw NSError(
                domain: "RiftVMOmarchyOwnerProvisioning",
                code: 1,
    ...
    public func requestAgentRestart() async throws {
        ...
        guard result.success else {
            throw NSError(
                domain: "RiftVMOmarchyAgentRestart",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: result.message]
            )
        }
    ...
    public func currentDesktopNotifications() async throws -> [VMOmarchyDesktopNotification] {
        ...
        guard result.success else {
            throw NSError(
                domain: "RiftVMOmarchyNotifications",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: result.message]
            )
        }
    ...
    public func verifySharedFolderRoundTrip(
        ...
        let hostData = Data("riftvm-host-to-guest:\(nonce)".utf8)
        let guestData = Data("riftvm-guest-to-host:\(nonce)".utf8)
        let importSource = layout.diagnostics.appending(path: ".riftvm-import-source-\(nonce).marker")
        var importedURL: URL?
        defer {
    ...
    private static func requireSuccess(_ result: VMGuestAgentTransferResult) throws {
        guard result.success else {
            throw NSError(
                domain: "RiftVMOmarchySharedFolderProbe",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: result.message]
            )
        }
