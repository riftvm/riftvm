...
class MachineSnapshotsViewStateObject {
        ...
        Task.detached {
            ...
            if keepCurrentState {
                ...
                if case let .failure(error) = safetyResult {
                    await MainActor.run {
                        guard self.isCurrent(control) else { return }
                        self.fail("Restore cancelled because RiftVM could not snapshot the current state: \(error)")
                        self.reload()
                    }
                    return
                }
    ...
    func cancelOperation() {
        ...
        workingDetail = String(localized: "Cancellation requested. RiftVM will stop at the next safe boundary.")
    }
...
struct MachineSnapshotsView: View {
    ...
    var body: some View {
                ...
                if state.selectedBackend == .diskImageKitLayered,
                   state.maximumASIFLayerDepth > 0 {
                    Text("· \(state.maximumASIFLayerDepth)-layer depth")
                        ...
                        .help(
                            state.maximumASIFLayerDepth >= VMSnapshotManager.recommendedMaximumASIFLayerDepth
                                ? "This snapshot chain is deep. Audit it before adding more snapshots, keep an exported backup, and remove unneeded leaf branches. RiftVM does not compact layered ASIF disks in place."
        ...
        .confirmationDialog(
            ...
            if let preview = state.cleanupPreview {
                Text(
                    "RiftVM verified that no active branch or saved snapshot references these layers. " +
...
private struct MachineSnapshotRestoreReviewView: View {
    ...
    private var statusMessage: String {
        ...
        case .some(true):
            "There is enough reported space. RiftVM will verify capacity again before changing the machine."
        case .some(false):
            "Restore is unavailable. Free at least \(displaySize(missingBytes)) and try again."
        case .none:
            "macOS did not report available capacity. RiftVM will check again before changing the machine."
        }
    }
    ...
    private var preservationMessage: String {
        if review.keepCurrentState {
            "RiftVM will create a recovery snapshot before restoring. The estimate is conservative; APFS copy-on-write may use less physical space."
        } else {
