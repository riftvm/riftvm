...
enum VMUSBFailureGuidance {
    ...
    static func message(for kind: VMUSBFailureKind, fallback: String) -> String {
        switch kind {
        case .listenerAlreadyRegistered:
            String(localized: "Accessory Access is already active in another RiftVM virtual machine window. Close that virtual machine or release its USB accessories, then try again.")
        case .accessoryNotAccessible:
            String(localized: "This accessory is being used by macOS or another app. Release it from the accessory menu, then choose it for RiftVM again.")
        case .invalidAccessoryState:
            String(localized: "This accessory is no longer in a state that can be attached. Disconnect it from the Mac, reconnect it, and approve it for RiftVM again.")
        case .controllerNotFound:
        ...
        case .deviceNotFound:
            String(localized: "The USB device is no longer connected to this virtual machine. Reconnect it to the Mac and approve it for RiftVM again.")
        case .internalFailure:
            String(localized: "Accessory Access could not complete the operation. Try again; if it repeats, restart RiftVM and reconnect the accessory.")
        case .other:
            fallback
        }
    }
...
enum VMReleaseSmokeTest {
    static let vmPathEnvironmentKey = "RIFTVM_RELEASE_SMOKE_VM"
    static let resultPathEnvironmentKey = "RIFTVM_RELEASE_SMOKE_RESULT"
    static let processIDPathEnvironmentKey = "RIFTVM_RELEASE_SMOKE_PID"
    static let requireGuestAgentEnvironmentKey = "RIFTVM_RELEASE_REQUIRE_GUEST_AGENT"
    static let requireGuestInputEnvironmentKey = "RIFTVM_RELEASE_REQUIRE_GUEST_INPUT"
    ...
    static let requireASIFStorageEnvironmentKey = "RIFTVM_RELEASE_REQUIRE_ASIF_STORAGE"
    static let requireVMNetEnvironmentKey = "RIFTVM_RELEASE_REQUIRE_VMNET"
    static let requireConfigurationLabelEnvironmentKey = "RIFTVM_RELEASE_REQUIRE_CONFIGURATION_LABEL"
...
struct VMReleaseSnapshotTestConfiguration: Equatable {
    ...
    static let actionEnvironmentKey = "RIFTVM_RELEASE_SNAPSHOT_ACTION"
    static let vmPathEnvironmentKey = "RIFTVM_RELEASE_SNAPSHOT_VM"
    static let resultPathEnvironmentKey = "RIFTVM_RELEASE_SNAPSHOT_RESULT"
    static let snapshotIDEnvironmentKey = "RIFTVM_RELEASE_SNAPSHOT_ID"
...
struct VMReleasePortabilityTestConfiguration: Equatable {
    ...
    static let actionEnvironmentKey = "RIFTVM_RELEASE_PORTABILITY_ACTION"
    static let inputEnvironmentKey = "RIFTVM_RELEASE_PORTABILITY_INPUT"
    static let outputEnvironmentKey = "RIFTVM_RELEASE_PORTABILITY_OUTPUT"
    static let resultEnvironmentKey = "RIFTVM_RELEASE_PORTABILITY_RESULT"
...
enum VMGuestProvisioningStartFailureGuidance {
    static func message(retryWasPrepared: Bool) -> String {
        ...
        return String(localized: "macOS guest provisioning did not start, and RiftVM could not prepare a safe retry. Close this VM window, reopen it, and review the provisioning status before trying again.")
    }
}
...
enum VMSavedStateStore {
    ...
    static func coldBootNotice(reason: String) -> String {
        "RiftVM could not safely resume the saved session because \(reason). " +
