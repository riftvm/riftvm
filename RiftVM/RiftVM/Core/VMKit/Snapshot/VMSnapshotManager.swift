...
class VMSnapshotManager {
    ...
    private static func acquireMutationLease(vmRootPath: URL) -> VMOSResult<VMSnapshotMutationLease, String> {
        ...
        guard descriptor >= 0 else {
            ...
            return .failure("RiftVM could not open the snapshot operation lock: \(String(cString: strerror(errno))).")
        }
        var status = stat()
        guard fstat(descriptor, &status) == 0 else {
            ...
            close(descriptor)
            releaseMutationReservation(key)
            return .failure("RiftVM could not inspect the snapshot operation lock: \(String(cString: strerror(lockError))).")
        }
        guard (status.st_mode & S_IFMT) == S_IFREG else {
            close(descriptor)
            releaseMutationReservation(key)
            return .failure("The snapshot operation lock is not a regular file. RiftVM left it untouched.")
        }
        guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else {
            ...
            if lockError == EWOULDBLOCK {
                return .failure("Another RiftVM process is already changing snapshots for this virtual machine. Wait for it to finish, then try again.")
            }
            return .failure("RiftVM could not acquire the snapshot operation lock: \(String(cString: strerror(lockError))).")
        }
            ...
            do {
                ...
                return .failure(
                    "The interrupted snapshot restore journal is unreadable. "
                        + "RiftVM preserved the current machine, recovery backup, and journal to avoid guessing which state is authoritative."
                )
            }
        } else {
            transaction = nil
    ...
    static func validateExistingASIFBaseDependency(baseURL: URL, vmRootPath: URL) -> VMOSResultVoid {
        ...
        guard sameFileSystemLocation(baseURL.standardizedFileURL.deletingLastPathComponent(), vmRootPath),
            ...
            return .failure(
                ...
                "active or saved layers depend on it. RiftVM did not create a replacement. Restore the " +
        ...
        if #available(macOS 27.0, *), !activeLayerPaths.isEmpty {
            do {
                ...
                return .failure(
                    ...
                    "is missing or damaged. RiftVM did not modify the disk chain. Restore the original " +
            ...
            for disk in snapshot.diskLayers {
                ...
                if disk.layerPaths.count >= recommendedMaximumASIFLayerDepth {
                    warnings.append(
                        ...
                        "leaf branches. RiftVM does not compact layered ASIF disks in place."
                    )
                }
#if canImport(DiskImageKit)
    ...
    /// no restore transaction is in flight, and the file is in RiftVM's UUID
        ...
        do {
            ...
            for name in quarantinedNames {
                ...
                guard sameFileSystemLocation(item.deletingLastPathComponent(), quarantine),
                    ...
                    throw VMSnapshotError.message("The cleanup quarantine contains an unmanaged item; RiftVM left it untouched.")
                }
            }
            if transaction?.phase == "committed" {
                guard Set(quarantinedNames).isSubset(of: Set(transaction?.layerNames ?? [])) else {
                    throw VMSnapshotError.message("The cleanup quarantine does not match its committed journal; RiftVM left it untouched.")
                }
