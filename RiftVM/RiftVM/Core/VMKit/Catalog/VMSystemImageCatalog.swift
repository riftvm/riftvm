...
struct VMPreinstalledImageCatalogItem: Identifiable, Hashable {
    ...
    static let omarchy = VMPreinstalledImageCatalogItem(
        ...
        manifestURL: URL(string: "https://github.com/riftvm/riftvm-omarchy-aarch64-image/releases/download/v4.0.3-riftvm.5/riftvm-omarchy-factory-manifest.json")!,
        downloadSize: 5_238_685_696
    )
}
    ...
    static func preparePath(fileName: String) -> URL? {
        ...
        if !FileManager.default.fileExists(atPath: dir.path(percentEncoded: false)) {
            do {
                ...
                RiftVMLog.error("Failed to create image store directory: \(error.localizedDescription)", logger: RiftVMLog.storage)
                return nil
            }
        }
