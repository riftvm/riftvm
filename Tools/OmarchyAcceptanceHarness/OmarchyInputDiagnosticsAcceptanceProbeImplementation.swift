...
enum OmarchyInputDiagnosticsAcceptanceProbe {
    ...
    static func runPinyin(
        ...
        (
          while [ ! -f "$d/capture" ]; do sleep 0.05; done
        ...
        guard engine.trimmingCharacters(in: .whitespacesAndNewlines) == "pinyin" else {
            throw ProbeError.stageTimeout("active Pinyin engine")
        }
        try await client.typeUSASCII("nihaoo")
        ...
        guard committed == "你好" else { throw ProbeError.unexpectedPinyinCommit }
        let report: [String: Any] = [
            "result": "passed", "engine": "fcitx5-pinyin", "committed": committed,
            "backspaceVerified": true, "backspacePhase": "preedit", "route": "guest-agent-uinput",
            "candidateScreenshot": directory.appending(path: "candidates.png").path,
        ]
        try JSONSerialization.data(withJSONObject: report, options: [.prettyPrinted, .sortedKeys])
            .write(to: diagnosticsDirectory.appending(path: "pinyin-input.json"), options: .atomic)
    }
