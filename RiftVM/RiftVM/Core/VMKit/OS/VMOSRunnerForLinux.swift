...
class VMOSRunnerForLinux : VMOSRunner {
    ...
    func createConfiguration(
        ...
        if (model.config.linuxFeatures ?? .legacy).virtioSocketEnabled {
            guard !model.config.directorySharingDevices.contains(where: {
                ...
                return .failure("The directory-sharing tag '\(VMGuestAgentEnrollmentStore.sharedDirectoryTag)' is reserved for the RiftVM guest agent.")
            }
    ...
    /// initramfs changes, and guest drivers in the complete RiftVM runtime. It is
    ...
    private func createDiagnosticDirectBootLoader() -> VZLinuxBootLoader? {
        ...
        guard process.arguments.contains("--riftvm-headless") || VMReleaseSmokeTest.configuration() != nil,
              let kernelPath = process.environment["RIFTVM_EXPERIMENTAL_LINUX_KERNEL"],
              let commandLine = process.environment["RIFTVM_EXPERIMENTAL_LINUX_COMMAND_LINE"],
              !kernelPath.isEmpty, !commandLine.isEmpty else { return nil }

        let bootLoader = VZLinuxBootLoader(kernelURL: URL(fileURLWithPath: kernelPath))
        if let initrdPath = process.environment["RIFTVM_EXPERIMENTAL_LINUX_INITRD"],
           !initrdPath.isEmpty {
