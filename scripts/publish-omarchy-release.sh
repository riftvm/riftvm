...
soak_observation=${12:-}
release_branch=${RIFTVM_OMARCHY_RELEASE_BRANCH:-main}
...
for variable in APPLE_ID APPLE_TEAM_ID APPLE_SPECIFIC_PASSWORD RIFTVM_OMARCHY_FACTORY_PUBLIC_KEY_BASE64; do
...
state_root=${RIFTVM_OMARCHY_RELEASE_STATE_DIR:-${TMPDIR:-/tmp}/riftvm-omarchy-release-state}
release_dir="$state_root/$version"
derived_data="$release_dir/DerivedData"
archive="$release_dir/RiftVM-Omarchy-$version.zip"
...
if [[ -f $source_file && $(tr -d '\r\n' <"$source_file") == "$source_revision" && \
  ...
  echo "Reusing verified RiftVM Omarchy $version candidate from $release_dir"
else
  rm -f "$archive" "$checksum" "$source_file" "$notarized_file"
  rm -rf "$derived_data"
  RIFTVM_SIGNING_IDENTITY="$signing_identity" \
  RIFTVM_OMARCHY_DERIVED_DATA="$derived_data" \
...
xattr -w com.apple.quarantine "0081;$(printf '%x' "$(date +%s)");RiftVMOmarchyRelease;" \
  "$install_check/RiftVM Omarchy.app"
RIFTVM_OMARCHY_LAUNCH_TIMEOUT=${RIFTVM_OMARCHY_LAUNCH_TIMEOUT:-10} \
  "$project_root/scripts/verify-omarchy-release-gui.sh" \
    "$install_check/RiftVM Omarchy.app" "$version" "$source_revision"
...
if pgrep -x 'RiftVM Omarchy' >/dev/null 2>&1; then
  fail "RiftVM Omarchy remained running before cold archive verification" 70
fi
rm -rf "$install_check"
mkdir -p "$install_check"
ditto -x -k "$archive" "$install_check"
cold_settle_seconds=${RIFTVM_OMARCHY_COLD_VERIFY_SETTLE_SECONDS:-30}
[[ $cold_settle_seconds =~ ^[0-9]+$ ]] || \
  fail "RIFTVM_OMARCHY_COLD_VERIFY_SETTLE_SECONDS must be a non-negative integer" 64
...
"$project_root/scripts/verify-omarchy-release-app.sh" \
  "$install_check/RiftVM Omarchy.app" "$version" "$source_revision" clean
spctl --assess --type execute --verbose=4 "$install_check/RiftVM Omarchy.app"

if [[ $mode == prepare ]]; then
  echo "Prepared notarized RiftVM Omarchy candidate: $archive"
...
if gh release view "$tag" --repo riftvm/riftvm >/dev/null 2>&1; then
  download=$(mktemp -d "${TMPDIR:-/tmp}/riftvm-omarchy-published.XXXXXX")
  trap 'rm -rf "$download"' EXIT
  gh release download "$tag" --repo riftvm/riftvm --pattern "$(basename "$archive")*" --dir "$download"
  ...
  gh release create "$tag" "$archive" "$checksum" "$factory_manifest" \
    --repo riftvm/riftvm --verify-tag --generate-notes --title "RiftVM Omarchy $version"
fi

echo "Published RiftVM Omarchy $version after notarized real-guest acceptance."
