...
@MainActor
enum RiftVMDiagnostics {
    static func export() throws -> URL {
        let panel = NSSavePanel()
        panel.title = "Export RiftVM Diagnostics"
        panel.nameFieldStringValue = "RiftVM-Diagnostics-\(ISO8601DateFormatter().string(from: Date()).prefix(10)).txt"
    ...
    private static var customVirGLIncluded: Bool {
        #if canImport(RiftVMVirGLRuntime)
        true
        #else
        false
        #endif
...
public class MacKitUtil {
    ...
    public static func saveImage(_ image: NSImage, atUrl url: URL) {
        ...
        do {
            ...
            RiftVMLog.error("Failed to save image: \(error.localizedDescription)", logger: RiftVMLog.storage)
        }
    }
}
