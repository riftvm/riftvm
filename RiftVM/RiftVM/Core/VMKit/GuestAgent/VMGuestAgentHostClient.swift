...
final class VMGuestAgentHostClient {
        ...
        while !Task.isCancelled, !pendingInputBatches.isEmpty {
            ...
            do {
                ...
                guard result.success else {
                    throw NSError(
                        domain: "RiftVMGuestInput", code: 3,
    ...
    func verifyInputInjection() async throws {
        guard capabilities.contains("input-uinput-v1") else {
            throw NSError(
                domain: "RiftVMGuestInput", code: 1,
        ...
        guard result.success else {
            throw NSError(
                domain: "RiftVMGuestInput", code: 2,
                userInfo: [NSLocalizedDescriptionKey: result.message]
            )
        }
    }
    ...
    func injectVisibleInputFixture() async throws {
        ...
        guard result.success else {
            throw NSError(
                domain: "RiftVMGuestInput", code: 4,
                userInfo: [NSLocalizedDescriptionKey: result.message]
            )
        }
    }
    ...
    private func inputDiagnostic(_ message: String) {
        guard ProcessInfo.processInfo.environment["RIFTVM_INPUT_DIAGNOSTICS"] == "1" else { return }
    ...
    private func requireSuccess(_ result: VMGuestAgentTransferResult) throws {
        guard result.success else {
            throw NSError(domain: "RiftVMGuestAgentTransfer", code: 1, userInfo: [NSLocalizedDescriptionKey: result.message])
        }
    }
