...
Omarchy uses Custom VirGL. The legacy general-VM commands below require fixtures
with a top-level `config.json`; they are not Omarchy machine commands. Keep CPU,
memory, guest packages, resolution, and workload identical, and record the actual
renderer.
...
```sh
scripts/verify-virgl-performance.sh /tmp/virgl.txt
```
...
The idle-desktop sample is captured from a running machine with the app's own
diagnostics (see the capture section above) and checked by the same verifier:

```sh
scripts/verify-virgl-performance.sh /path/to/report.txt
```

The capture covers the idle baseline only. Interactive scrolling, workspace
movement, and video remain human-driven representative workloads and must not be
...
## September 2, 2026 signed idle baseline
...
a 30-second `hyprland-idle` workload:

| Metric | Custom VirGL |
| --- | ---: |
| Average host CPU | 8.7% |
| Average host RSS | 180.8 MiB |
| Average FPS | 60.7 |
| Minimum five-second FPS | 59.4 |
| Average presentation | 0.71 ms |
| Worst-window P95 presentation | 1.15 ms |
| Absolute peak presentation | 2.80 ms |
| Drawable misses / presentation failures | 0 / 0 |

The measured CPU and RSS are within the checked-in idle budgets. This proves a
repeatable idle baseline, not interactive browser, video, scrolling, or
...
## September 3, 2026 signed idle captures

The current RiftVM 2.0.0 Developer ID candidate passed two more same-fixture
`hyprland-idle` captures on macOS 27.0 (26A5425a), Mac15,6. The first
30-second capture reported 15.0% average host CPU and 186.3 MiB average RSS.
Custom VirGL sustained 60.8 FPS with zero drawable misses or presentation
failures; average, worst-window P95, and absolute-peak presentation were
1.12 ms, 1.52 ms, and 3.34 ms.

A 60-second repeat reduced the average to 11.5% CPU; average RSS was 190.7 MiB.
Across 12 VirGL windows it sustained 60.4 FPS, zero misses, and zero failures,
with 0.84 ms average and 1.31 ms worst-window P95 presentation. One 34.58 ms
sample occurred in the first warm-up window; every later window peaked at or
below 3.32 ms. Both runs passed the checked-in CPU, memory, cadence, and latency
