...
enum RiftVMExecutableLocation {
    ...
    static func hostAppExecutable(
        ...
        let appExecutable = helperDirectory.deletingLastPathComponent().appendingPathComponent("MacOS/RiftVM")
        return fileManager.isExecutableFile(atPath: appExecutable.path) ? appExecutable : nil
    }
}

public enum RiftVMCLIExit: Int32, Sendable {
    case success = 0
...
public struct RiftVMCLIResponse: Codable, Equatable, Sendable {
    public let schemaVersion: Int
    public let command: String
    public let success: Bool
    public let result: JSONValue?
    public let error: RiftVMCLIErrorPayload?
    ...
    public init(command: String, code: String, message: String) {
        ...
        success = false
        result = nil
        error = RiftVMCLIErrorPayload(code: code, message: message)
    }
}

public struct RiftVMCLIErrorPayload: Codable, Equatable, Sendable {
...
public struct RiftVMMachineSummary: Codable, Equatable, Sendable {
...
public struct RiftVMHeadlessRecord: Codable, Equatable, Sendable {
    public let schemaVersion: Int
...
public struct RiftVMPreinstalledImageManifest: Codable, Equatable, Sendable {
    ...
    public let minimumRiftVMVersion: String
    ...
    public func validate(minimumDiskSize: UInt64 = 10 * 1024 * 1024 * 1024) throws {
        ...
        guard Self.isVersion(minimumRiftVMVersion, atMost: Self.runningVersion) else {
            throw ValidationError("This image requires RiftVM \(minimumRiftVMVersion) or newer; this installation is \(Self.runningVersion).")
        }
    }
...
private struct RiftVMSharedRuntimeRecord: Codable {
...
public struct RiftVMMachineInspector {
    ...
    public func inspect(_ machineURL: URL) -> RiftVMMachineSummary {
...
public struct RiftVMCLI {
    ...
    public func run(arguments: [String], environment: [String: String] = ProcessInfo.processInfo.environment) -> (RiftVMCLIExit, RiftVMCLIResponse) {
        ...
        case "inspect", "validate":
            ...
            } catch let error as RiftVMMachineInspector.InspectionError {
    ...
    public func encode(_ response: RiftVMCLIResponse) throws -> Data {
    ...
    private func parse(arguments: [String], environment: [String: String]) throws -> Parsed {
        guard let command = arguments.first else { throw ParseError("Usage: riftvm <list|inspect|validate|doctor> [machine] [--root path]") }
    ...
    private func installImage(_ parsed: Parsed) -> (RiftVMCLIExit, RiftVMCLIResponse) {
        guard let manifestPath = parsed.target, let image = parsed.image, let destination = parsed.destination else {
            return (.invalidArguments, .init(command: "install-image", code: "invalid_arguments",
                message: "Usage: riftvm install-image <manifest.json> --image <disk.raw> --destination <machine.riftvm> [--thumbnail image.png] [--name name] [--timeout seconds]"))
        }
        let manifestURL = URL(fileURLWithPath: manifestPath).standardizedFileURL
        guard destination.pathExtension.lowercased() == "riftvm" else {
            return (.invalidArguments, .init(command: "install-image", code: "invalid_destination", message: "Destination must use the .riftvm extension."))
        }
        ...
        do {
            let manifest = try RiftVMPreinstalledImageManifest.load(from: manifestURL, minimumDiskSize: minimumPreinstalledDiskSize)
            ...
            guard let executable = hostAppExecutable() else {
                return (.unavailable, .init(command: "install-image", code: "host_app_unavailable", message: "Could not locate the RiftVM application executable."))
            }
            ...
            process.arguments = ["--riftvm-install-preinstalled-image", manifestURL.path, "--image", image.path,
            ...
            guard process.terminationStatus == 0 else {
                ...
                return (.internalError, .init(command: "install-image", code: "install_failed", message: message.isEmpty ? "RiftVM could not install the image." : message))
            }
        ...
        } catch let error as RiftVMPreinstalledImageManifest.ValidationError {
    ...
    private func start(_ parsed: Parsed) -> (RiftVMCLIExit, RiftVMCLIResponse) {
        ...
        do {
            ...
            process.executableURL = executable
            process.arguments = ["--riftvm-headless", machine.path, "--state-file", stateURL.path,
    ...
    private func status(_ parsed: Parsed) -> (RiftVMCLIExit, RiftVMCLIResponse) {
    ...
    private func stop(_ parsed: Parsed) -> (RiftVMCLIExit, RiftVMCLIResponse) {
    ...
    private func unexpectedTarget(_ command: String) -> (RiftVMCLIExit, RiftVMCLIResponse) {
    ...
    private func isExpectedHeadlessProcess(_ record: RiftVMHeadlessRecord, machine: URL) -> Bool {
        let pid = record.pid
    ...
    private func readActiveSharedRuntime(for machine: URL) -> RiftVMSharedRuntimeRecord? {
        ...
        guard errno == EWOULDBLOCK, let data = try? Data(contentsOf: metadataURL),
              let record = try? JSONDecoder().decode(RiftVMSharedRuntimeRecord.self, from: data),
              record.schemaVersion == 1, record.machinePath == key,
              isExpectedAppProcess(record.pid) else { return nil }
        return record
    }

    private func sharedRuntimeJSON(_ value: RiftVMSharedRuntimeRecord) -> JSONValue {
    ...
    private func headlessJSON(_ value: RiftVMHeadlessRecord) -> JSONValue {
        .object(["pid": .number(Double(value.pid)), "machinePath": .string(value.machinePath),
                 "phase": .string(value.phase), "message": value.message.map(JSONValue.string) ?? .null,
                 "updatedAt": .string(ISO8601DateFormatter().string(from: value.updatedAt))])
    }

    private func summaryJSON(_ value: RiftVMMachineSummary) -> JSONValue {
