...
`RiftVMCore` package from the parent repository while the product boundary is
...
The app owns one Host folder at `~/Library/Application Support/RiftVM
...
`/mnt/riftvm-shared`; the Integration menu reports it as available only after the
