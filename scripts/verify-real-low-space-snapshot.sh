...
work_root="$(mktemp -d "${TMPDIR:-/tmp}/riftvm-real-low-space.XXXXXX")"
...
diskutil image create blank --format RAW --size 1100m --fs APFS \
  --volumeName RiftVMNearFull "$image_path" >/dev/null
...
# under RiftVM's 1 GiB safety reserve without filling the user's live volume.
...
RIFTVM_REAL_LOW_SPACE_VOLUME="$mount_path" \
