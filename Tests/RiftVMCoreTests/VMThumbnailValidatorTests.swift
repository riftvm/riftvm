...
final class VMThumbnailValidatorTests: XCTestCase {
    ...
    func testPerMachineStylePreferenceKeyIsStableAndDistinct() {
        let first = URL(fileURLWithPath: "/tmp/Alpha.riftvm")
        let equivalent = URL(fileURLWithPath: "/tmp/folder/../Alpha.riftvm")
        let second = URL(fileURLWithPath: "/tmp/Beta.riftvm")
