...
derived_data=${RIFTVM_OMARCHY_DERIVED_DATA:-}
...
[[ $factory_public_key =~ ^[A-Za-z0-9+/]{43}=$ ]] || {
  echo "RIFTVM_OMARCHY_FACTORY_PUBLIC_KEY_BASE64 must contain a 32-byte Ed25519 public key" >&2
  exit 78
}
decoded_key=$(mktemp "${RUNNER_TEMP:-/tmp}/riftvm-omarchy-public-key.XXXXXX")
trap 'rm -f "$decoded_key"' EXIT
printf '%s' "$factory_public_key" | base64 -D >"$decoded_key" 2>/dev/null || {
  echo "RIFTVM_OMARCHY_FACTORY_PUBLIC_KEY_BASE64 is invalid base64" >&2
  exit 78
}
[[ $(wc -c <"$decoded_key" | tr -d ' ') == 32 ]] || {
  echo "RIFTVM_OMARCHY_FACTORY_PUBLIC_KEY_BASE64 must decode to 32 bytes" >&2
  exit 78
}
...
if [[ -z $derived_data ]]; then
  derived_data=$(mktemp -d "${RUNNER_TEMP:-/tmp}/riftvm-omarchy-release-derived.XXXXXX")
fi
[[ ! -e "$derived_data/Build/Products/Release/RiftVM Omarchy.app" ]] || {
  echo "release derived data must be empty: $derived_data" >&2
  exit 65
}

project_file="$project_root/RiftVMOmarchy/RiftVMOmarchy.xcodeproj/project.pbxproj"
[[ -f $project_file ]] || { echo "generated RiftVM Omarchy project is missing" >&2; exit 65; }
project_hash_before=$(shasum -a 256 "$project_file" | awk '{print $1}')
(cd "$project_root/RiftVMOmarchy" && xcodegen generate)
project_hash_after=$(shasum -a 256 "$project_file" | awk '{print $1}')
[[ $project_hash_before == "$project_hash_after" ]] || {
  echo "generated RiftVM Omarchy project is stale" >&2
  exit 65
}

if [[ -n ${RIFTVM_SIGNING_IDENTITY:-} ]]; then
  archive_path="$derived_data/RiftVMOmarchy.xcarchive"
  export_path="$derived_data/DeveloperIDExport"
  xcodebuild archive \
    -project "$project_root/RiftVMOmarchy/RiftVMOmarchy.xcodeproj" \
    -scheme 'RiftVM Omarchy' \
    ...
    RIFTVM_OMARCHY_FACTORY_PUBLIC_KEY_BASE64="$factory_public_key" \
  ...
  xcodebuild -exportArchive \
    ...
    -allowProvisioningUpdates
  app_path="$export_path/RiftVM Omarchy.app"
  ...
  xcodebuild \
    ...
    MARKETING_VERSION="$version" \
    build
  app_path="$derived_data/Build/Products/Release/RiftVM Omarchy.app"
  signing_identity=-
fi
...
if [[ $signing_identity == - ]]; then
  signing_options+=(--entitlements "$project_root/RiftVMOmarchy/Resources/RiftVMOmarchy.entitlements" --timestamp=none)
else
  signing_options+=(--entitlements "$project_root/RiftVMOmarchy/Resources/RiftVMOmarchy.entitlements" --options runtime --timestamp)
fi
...
if [[ $signing_identity != - ]]; then
  sleep "${RIFTVM_OMARCHY_SIGNATURE_SETTLE_SECONDS:-30}"
  "$project_root/scripts/verify-omarchy-release-app.sh" \
    "$app_path" "$version" "$source_revision" "$source_tree_state"
fi

archive="RiftVM-Omarchy-$version.zip"
ditto -c -k --sequesterRsrc --keepParent "$app_path" "$output_dir/$archive"
roundtrip=$(mktemp -d "${RUNNER_TEMP:-/tmp}/riftvm-omarchy-release-roundtrip.XXXXXX")
trap 'rm -f "$decoded_key"; rm -rf "$roundtrip"' EXIT
ditto -x -k "$output_dir/$archive" "$roundtrip"
"$project_root/scripts/verify-omarchy-release-app.sh" \
  "$roundtrip/RiftVM Omarchy.app" "$version" "$source_revision" "$source_tree_state"
