...
### 2026-09-07 — Signed build 19 GUI duplication and boot

- Built and installed signed build 19 from clean a98084c6c73abbe3cac6998d7704228fc8de4442. ZIP SHA-256: c8ee0d2fde25145d1a890901f89190e4661d9ff879e460ed36aaede7c0ac7694. Build resource/metadata/entitlement checks and strict installed signature verification passed. Previous App was quit normally with no VM processes running.
- Used the new unified library Duplicate Workspace action on the retained Debian fixture. The native destination panel created Build19-Duplicate and the library grew from seven to eight. Verified distinct workspace/hardware identities, correct new name, relative bundled ISO and matching media SHA-256, while the source retained its original absolute media reference.
- Changed only the disposable copy to speaker-only before boot. GUI start reached installed Debian 13 tty login. Runtime file handles referenced the duplicate's own ASIF layer chain and bundled ISO. Normal GUI shutdown removed the VM runtime. Removed the duplicate from the library and deleted its temporary bundle only after stopped status and process exit were verified. Original guests and installers were retained. No completed guest login or historical snapshot restore is claimed.
- Build 19 still requires the full final-candidate matrix; build 18 CLI evidence is not relabeled as a build 19 pass. Input responsiveness, guest-data rollback and remaining integration/distribution gates remain open.

### 2026-09-07 — Reject host-marker rollback evidence; guest sandbox limitation

- The inherited rollback acceptance tool modified a Host file beside Disk.asif. That does not prove a Guest filesystem rollback. Replaced that path with a bounded, temporary-workspace-only /var/lib marker probe over authenticated transfer, with actual Guest bytes read back across boots and no passing report on failure.
- A signed development helper on an isolated copy of Omarchy New reached the Agent, but writes to /var/lib were rejected as read-only. Inspection confirmed the image service uses ProtectSystem=strict and limits writes to /run and the shared mount. This is expected sandbox enforcement, not a recovery failure. Guest permissions were not weakened. No snapshot/restore pass is claimed.
- Corrected the probe to allow marker replacement only after checking its expected previous contents, and to complete authenticated normal shutdown even when a data operation fails. Compilation passed; a second controlled run returned the same expected rejection only after stopped state, left no VM process and generated no passing JSON. Deleted only the stopped disposable fixture; original guests and diagnostic logs remain.
- Guest-disk rollback still needs a desktop-user-session path. Host/shared-folder markers cannot satisfy this gate. Installed build 19 does not include these development-tool changes.

### 2026-09-07 — Real VirtioFS permission and removal verification

- A signed temporary backend helper built against ede61c7 used an isolated copy of Omarchy New and two empty temporary host directories. It exercised the actual Virtualization.framework multi-directory share and authenticated Guest transfer service, not a mocked filesystem.
- Both read-only and read-write grants returned the Host marker. The read-only share rejected root Agent writes with operation not permitted; the read-write share completed Guest-to-Host writes. After normal shutdown, removed the read-only grant and restarted. Its former Guest path returned no such file, while the retained read-write grant still passed both directions. Both Host preserve.txt files remained byte-identical.
- Both VM passes completed normal shutdown; no virtualization runtime remained. Removed only the disposable VM bundle. Retained the helper source under /private/tmp/riftvm-folder-probe and its logs/results under /private/tmp/riftvm-folder-acceptance19. Result JSON SHA-256: bb9d53a0eb5d14a13d69147513d788713897e0ee4b82acb084ba8f6e4b78b841. This is backend runtime evidence, not a claim that the final signed App's folder picker or full mixed-profile acceptance has passed.

### 2026-09-07 — App-targeted Command event handling

- Build 19 again completed a visible Omarchy New cold login, and its accessibility permission banner disappeared. App-targeted Command+Return still produced no terminal. The bridge only monitored the session event tap, leaving directly delivered application events without the same Command-to-Agent route. This finding does not establish the cause of ordinary-character latency.
- Added a local key-event monitor using the existing focus policy and synthetic marker. Session-tap-consumed events do not arrive locally; replay events are passed through without looping. Stopping the bridge removes the monitor and clears held bookkeeping. The VM focus probe now requires running state, so stopped/paused workspaces do not capture host shortcuts.
- Added a regression for one balanced Command pair, ordinary input, synthetic replay and focus loss. All 59 native Omarchy integration tests passed using the repository virtualization-test entitlement and consistent ad-hoc test signing. Earlier attempts failed before test loading due to signing/provisioning configuration, not assertions. The installed App has not yet validated this change against a real Guest.

### 2026-09-07 — Direct responder input diagnosis and correction

- Installed signed build 20 from 71cfa8dde4dbd5cb4b9a2aa596b788787edc9168 (ZIP SHA-256 25ac12833ebb182b7cff06cc7e1138a645c136fe17a9a0ee3fcdfb8ef438f240) completed two Omarchy New cold logins. Command+Return remained ineffective. Menu text was partial before a later pointer action revealed the complete string; terminal text also appeared late. These observations do not establish whether Guest input processing or rendering causes the ordinary-input delay.
- A three-second host sample was predominantly idle in the AppKit event loop. Direct stderr capture with content-free timing enabled then showed Command events reaching the focused VM view in 0.9 and 5.1 ms, with zero local-monitor events. The previous local-monitor-only correction does not cover this responder delivery path. Evidence retained at /private/tmp/riftvm-candidate20/direct-input.log and /private/tmp/riftvm-build20-input-sample.txt.
- Wired direct keyDown/keyUp and key-equivalent delivery through the same focused Command bridge. The bridge still excludes ordinary keys, unfocused events and tagged replay. Added native regression coverage for direct responder pairs and key equivalents without starting an event monitor. Native Omarchy integration tests passed. The installed build 20 does not contain this correction; signed runtime verification is still required.
- Both diagnostic Guest runs ended through normal shutdown. No ordinary-input responsiveness or final release gate is marked passed by this change.

### 2026-09-07 — Signed build 21 direct Command runtime verification

- Built and installed signed build 21 from clean d901fe7 (ZIP SHA-256 cf141d653bbdd93d3777bfffa82d74ec6f10eeb1f2d9add04817e8422026c30b); resource, entitlement, round-trip and strict installed signature checks passed. All 60 native Omarchy integration tests passed before the build.
- Cold-started Omarchy New and completed visible login. One app-targeted Command+Return opened a terminal, unlike build 20. This verifies the direct responder bridge against the real Guest for that chord; it does not prove mixed-window modifier isolation.
- Ordinary terminal input remains failing acceptance: content-free diagnostics show events reaching the focused view within milliseconds, while the command echo RIFTVM21 was not fully visible after subsequent observations, and later only its initial character appeared. Return execution was not verified. Retained direct timing output at /private/tmp/riftvm-candidate21/direct-input.log. The issue remains downstream of the observed responder entry, with native virtual-keyboard delivery, Guest handling and presentation still to distinguish.
- Guest stopped normally after testing. Build 21 has not passed the full CLI matrix, notarization or publication gates. No final input or release acceptance pass is claimed.

### 2026-09-07 — Build 21 full CLI acceptance

- Ran scripts/verify-release-cli.sh against the installed signed build 21 and independent copies of the retained disk-only Debian fixture. The verifier exited 0, covering JSON commands, concurrent independent guests, duplicate-start rejection, SIGKILL runtime restart, corrupt saved-state preservation and explicit recovery, and EFI variable-store recovery. Four unanswered normal shutdown requests succeeded on the existing retry path.
- Verified no CLI or Virtualization VM runtime remained and the disposable .riftvm-cli-smoke.tLZ7G9 directory was removed. Original Guest and downloaded installers remain. Log retained at /private/tmp/riftvm-candidate21/cli-acceptance.log (SHA-256 e1fabf1a3355f0503fd82243b826c3c6b1b5d400c49a922e511278159b20f02c).
- This closes the build 21 CLI check only. Ordinary input latency, the remaining unified functional matrix and distribution checks remain open. Read-only inspection of image input classification and display watcher code did not identify a confirmed cause for the text delay; no speculative input change was made.

### 2026-09-07 — Preserve captured key ownership through release

- Code inspection found that captured Command-key releases were only consumed while Command remained held and the original workspace retained focus. Releasing Command first or changing focus could leave the bridge bookkeeping populated and leak an Agent-owned key-up into the native keyboard path.
...
### 2026-09-07 — Differentiate input and observation paths
...
### 2026-09-07 — Preserve historical ISO dependencies in exports

- Export previously embedded external installation media only for the current config. Historical snapshot configs could retain absolute ISO paths, so restoring imported history could depend on the original Mac. Export now embeds each historical config's ISO within its snapshot files, audits the existing manifest before mutation, and regenerates its manifest/allocated size only after a successful rewrite in private staging. Protected state, identity, parent links and disk-layer metadata are preserved.
- Export estimation includes historical external media. Independent clone estimation excludes discarded history, so an absent ISO referenced only by old history cannot prevent copying the current workspace. Export validation now audits snapshot history and checks its storage references, resolving layered block disks against the workspace and ordinary snapshot files against their saved root. Unreadable UUID history metadata is no longer silently skipped by portability.
- All 23 portability tests passed, including real filesystem export/import/restore after deleting the original historical ISO, source config/metadata preservation, protected history preservation, independent cloning with that missing historical ISO, and refusal to rehash pre-existing snapshot corruption as valid. Snapshot regression: 55 passed, 1 isolated real-low-space-volume test skipped because its required volume was not configured. Native unsigned App build passed.
- This is backend filesystem evidence, not a booted Guest or signed GUI portability pass. Installed build 23 does not include the change. Ordinary input and the remaining unified/distribution gates remain open.

### 2026-09-07 — Layered history portability regression
...
### 2026-09-07 — Build 24 CLI acceptance and owner-approved input deferral

- Installed signed build 24 completed scripts/verify-release-cli.sh against independent copies of Debian-DiskOnly.riftvm. The process exited 0 and reported successful JSON, concurrent guests, duplicate ownership rejection, SIGKILL restart, saved-state preservation/explicit recovery and EFI recovery. Four graceful shutdown retries occurred; this is not a forced-stop result. Log: /private/tmp/riftvm-candidate24/cli-acceptance.log. The temporary .riftvm-cli-smoke.uo7Ff2 directory was confirmed removed.
- The owner explicitly approved leaving the ordinary Omarchy input latency/unresponsive Return bug for a separate follow-up and requested completion of other work. This is a known issue accepted for 0.1.0, not a passing input test. It does not waive clipboard/notification isolation, guest-disk recovery, folder permissions, portability or distribution checks.
- A new independent Omarchy Clean workspace reached the first-owner setup screen. Owner creation and desktop acceptance have not completed; do not count this as a completed cold install. Existing workspaces were preserved.

### 2026-09-07 — Explicit input exception in release records
...
### 2026-09-07 — Folder permissions GUI and accessibility follow-up

- Signed build 24: stopped Omarchy Clean normally; its folder sheet initially showed no shared host folders. Added only /private/tmp/riftvm-build24-folder-ui via the native chooser and read-only confirmation, observed Read Only and the guest mount name, then removed the grant. The empty state returned and preserve.txt bytes remained unchanged. Closed the stopped workspace window. This proves the GUI grant/removal path and host preservation, not guest-side enforcement or completed owner setup.
- The AX tree exposed the permission controls as a single list row. Updated the row to contain its accessibility children, gave its access picker and removal button folder-specific labels, and used a borderless removal button to avoid list-row automatic button behavior. Native unsigned App build passed. Runtime accessibility verification on the next signed candidate remains required; installed build 24 does not contain this follow-up.

### 2026-09-07 — Build 25 runtime follow-up
...
### 2026-09-07 — Build 26 folder accessibility verified

- Installed build 26 after the owner explicitly authorized killing the stopped-guest build 25 App process. Strict installed signature passed. Initial AX reads timed out despite a normal main-thread event-loop sample; a subsequent read recovered without another restart. Cause remains unknown.
- On Omarchy Clean, stopped the guest normally and opened Folder Permissions. Added the retained temporary test folder read-only. The AX tree now separately exposes the access popup and folder-specific remove button. Opened the popup by element action, observed both choices, cancelled without broadening access, and removed the grant by its independent button. The empty state returned and preserve.txt remained byte-identical. Closed the stopped workspace window. This verifies the container accessibility fix, not guest-side read/write enforcement or completed owner setup.

### 2026-09-07 — Build 26 macOS window lifecycle and graceful shutdown

- Opened the retained MacOS-Speakers workspace in installed build 26 and observed the macOS desktop with Finder. Guest runtime PID was 13965. Closed its window and opened Workspaces with Command+0: the library still reported Running. Reopened the workspace and independently confirmed the same Guest PID 13965, without a duplicate runtime.
- Selected Shut Down in the host Power menu. The Guest displayed its own shutdown confirmation while the host showed its waiting overlay. Clicked the visible Guest Shut Down button; the host briefly reported that shutdown had not yet completed, then returned to the library with Stopped. PID 13965 was confirmed absent. No Force Stop or process kill was used for this Guest. This is a single macOS lifecycle pass, not mixed-profile quit, notification isolation or full display acceptance.
- Separately, closing the library and immediately invoking its Command+0 action restored the control-center window in build 26. This confirms that explicit command path; it does not prove every Dock/menu-bar reopen route or explain intermittent computer-control observation timeouts.
