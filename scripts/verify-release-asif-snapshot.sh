...
[[ -d "$app_path" && -d "$vm_path" ]] || fail "usage: $0 <RiftVM.app> <asif-linux-vm>"
...
fixture_root="$(mktemp -d "$fixture_parent/.riftvm-asif-snapshot-fixture.XXXXXX")"
fixture="$fixture_root/ASIF-Snapshot.riftvm"
result_file="$(mktemp "${TMPDIR:-/tmp}/riftvm-asif-snapshot-result.XXXXXX")"
launch_log="$(mktemp "${TMPDIR:-/tmp}/riftvm-asif-snapshot-launch.XXXXXX")"
cleanup() {
  rm -rf "$fixture_root"
...
run_action() {
  ...
  if ! open -n -g -W --stdout "$launch_log" --stderr "$launch_log" \
    --env "RIFTVM_RELEASE_SNAPSHOT_ACTION=$action" \
    --env "RIFTVM_RELEASE_SNAPSHOT_VM=$fixture" \
    --env "RIFTVM_RELEASE_SNAPSHOT_RESULT=$result_file" \
    --env "RIFTVM_RELEASE_SNAPSHOT_ID=$snapshot_id" \
    "$app_path"; then
    cat "$launch_log" >&2
