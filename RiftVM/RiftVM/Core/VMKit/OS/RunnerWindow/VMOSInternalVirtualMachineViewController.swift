...
public class VMOSInternalVirtualMachineViewController: NSViewController {
    ...
    private func installNetworkPowerObserversIfNeeded() {
        ...
        networkPowerObservers.append(center.addObserver(
            ...
            Task { @MainActor [weak self] in
                ...
                RiftVMLog.info("Host sleep suspended network recovery.", logger: RiftVMLog.network)
            }
        })
        networkPowerObservers.append(center.addObserver(
            ...
            Task { @MainActor [weak self] in
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [weak self] in
                    ...
                    if recoveryIndices.isEmpty {
                        RiftVMLog.info("Host wake preserved all network attachments.", logger: RiftVMLog.network)
                    }
    ...
    func updateRuntimeSharedFolders(_ devices: [VMModelFieldDirectorySharingDevice]) -> String? {
        ...
        guard let device = virtualMachine.directorySharingDevices
            ...
            return "The running virtual machine does not expose the RiftVM shared-folder device."
        }
    ...
    private func updateMachineStateSupport(for configuration: VZVirtualMachineConfiguration) {
        do {
            ...
            RiftVMLog.info("Machine-state save and restore is unavailable: \(error.localizedDescription)")
        }
    }
    ...
    private func recoverInvalidEFIBootIfPossible(error: Error, rootPath: URL, model: VMModel) -> Bool {
        ...
        do {
            ...
            RiftVMLog.info("Rejected EFI variable store was replaced; backup: \(backup?.path ?? "none")")
            let runner = VMOSRunnerFactory.getRunner(model.config.type)
        ...
        if recoveryMode {
            ...
            if graphicsBackend?.supportsMachineSaveRestore == false,
               FileManager.default.fileExists(atPath: model.savedMachineStateURL.path(percentEncoded: false)) {
                ...
                VMSavedStateStore.discardCommitted(stateURL: model.savedMachineStateURL)
                runtimeState?.updateMachineStateNotice(
                    String(localized: "The saved session used a graphics configuration that Custom VirGL cannot restore. RiftVM discarded it and started the virtual machine normally.")
                )
                RiftVMLog.info("Discarded saved state because Custom VirGL does not support machine-state restore.")
                ...
                case .incompatible(let reason):
                    VMSavedStateStore.discardCommitted(stateURL: model.savedMachineStateURL)
                    runtimeState?.updateMachineStateNotice(
                        "\(reason) RiftVM discarded the incompatible saved session and started the virtual machine normally."
                    )
                    RiftVMLog.info("Discarded incompatible saved state: \(reason)")
                }
            }
                        ...
                        Task { @MainActor in
                            ...
                            if let error {
                                ...
                                case .failure(let keychainError):
                                    ...
                                    RiftVMLog.error(keychainError, logger: RiftVMLog.lifecycle)
                                }
    ...
    private func startReleaseGuestAgentSmokeTest(_ configuration: VMReleaseSmokeTestConfiguration) {
        ...
        let uploadURL = FileManager.default.temporaryDirectory.appendingPathComponent("rift-agent-upload-\(token)")
        let downloadURL = FileManager.default.temporaryDirectory.appendingPathComponent("rift-agent-download-\(token)")
        let payload = Data("RiftVM real guest-agent integration \(token)\n".utf8)
        do {
        ...
        releaseSmokeGuestPath = "/tmp/rift-agent-integration-\(token)"
    ...
    private func performNetworkReconnect(deviceIndex: Int) {
        ...
        runtimeState?.updateNetworkRuntime(networkRuntimeTracker.state)
        RiftVMLog.info("Reconnecting network adapter \(deviceIndex + 1).", logger: RiftVMLog.network)
            ...
            Task { @MainActor in
                ...
                if let error {
                    ...
                    self.runtimeState?.updateMachineStateNotice(
                        "The virtual machine is still paused. RiftVM removed the incomplete saved-session result; try Save & Stop again."
                    )
                    return
                }
                self.releaseVirtualMachineAfterStop()
    ...
    private func captureScreenshot(synchronously: Bool = false, allowReplacement: Bool = false) {
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                screenshotCaptureInProgress = false
                RiftVMLog.error("Failed to capture VM thumbnail: \(error.localizedDescription)", logger: RiftVMLog.storage)
            }
        }
    }
...
extension VMOSInternalVirtualMachineViewController: VZVirtualMachineDelegate {
    ...
    public func virtualMachine(_ virtualMachine: VZVirtualMachine, networkDevice: VZNetworkDevice, attachmentWasDisconnectedWithError error: Error) {
        guard let deviceIndex = virtualMachine.networkDevices.firstIndex(where: { $0 === networkDevice }) else {
            RiftVMLog.error("An unknown network device disconnected: \(error.localizedDescription)", logger: RiftVMLog.network)
            return
        }
        networkReconnectTokens.removeValue(forKey: deviceIndex)
        networkRuntimeTracker.markDisconnected(
        ...
        Task { @MainActor [weak self] in
            ...
            if disposition == .attachInterrupted {
                self.notice = .attachFailed(
                    ...
                    detail: String(localized: "The accessory disconnected before the connection finished. Reconnect it to the Mac and approve it for RiftVM again.")
                )
