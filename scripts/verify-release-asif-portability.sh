...
work_root="$(mktemp -d "$fixture_parent/.riftvm-asif-portability.XXXXXX")"
source_vm="$work_root/Source.riftvm"
export_path="$work_root/Portable.riftvmexport"
imported_vm="$work_root/Imported.riftvm"
result_file="$(mktemp "${TMPDIR:-/tmp}/riftvm-portability-result.XXXXXX")"
launch_log="$(mktemp "${TMPDIR:-/tmp}/riftvm-portability-launch.XXXXXX")"
...
run_action() {
  ...
  if ! open -n -g -W --stdout "$launch_log" --stderr "$launch_log" \
    --env "RIFTVM_RELEASE_PORTABILITY_ACTION=$action" \
    --env "RIFTVM_RELEASE_PORTABILITY_INPUT=$input" \
    --env "RIFTVM_RELEASE_PORTABILITY_OUTPUT=$output" \
    --env "RIFTVM_RELEASE_PORTABILITY_RESULT=$result_file" \
    "$app_path"; then
    cat "$launch_log" >&2
...
RIFTVM_RELEASE_REQUIRE_ASIF_STORAGE=1 \
