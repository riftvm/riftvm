...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    private func runReleaseSnapshotTest(_ test: VMReleaseSnapshotTestConfiguration) {
        ...
        case .create:
            ...
            switch VMSnapshotManager.createSnapshot(
                ...
                name: "RiftVM signed release validation"
            ) {
    ...
    private func installPreinstalledImage(_ install: PreinstalledImageInstallConfiguration) {
        ...
        Task {
            ...
            switch result {
            case .success:
                print("Installed RiftVM machine: \(install.destinationURL.path)")
                fflush(stdout)
                exit(0)
            case .failure(let message):
                FileHandle.standardError.write(Data("RiftVM image installation failed: \(message)\n".utf8))
                exit(70)
            }
        }
    }
    ...
    private func performPreinstalledImageInstall(_ install: PreinstalledImageInstallConfiguration) async -> VMOSResultVoid {
        ...
        guard install.destinationURL.pathExtension.lowercased() == "riftvm" else {
            return .failure("Destination must use the .riftvm extension.")
        }
    ...
    private func writeHeadlessState(_ launch: HeadlessLaunchConfiguration, phase: String, message: String?) {
        ...
        do {
            ...
            RiftVMLog.error("Could not write headless state: \(error.localizedDescription)")
        }
    }
...
struct ReleaseFixtureCreationConfiguration {
    ...
    static var current: ReleaseFixtureCreationConfiguration? {
        ...
        guard let osValue = environment["RIFTVM_RELEASE_CREATE_OS"],
              let osType = VMOSType(rawValue: osValue),
              let imagePath = environment["RIFTVM_RELEASE_CREATE_IMAGE"], !imagePath.isEmpty,
              let destinationPath = environment["RIFTVM_RELEASE_CREATE_VM"], !destinationPath.isEmpty,
              let resultPath = environment["RIFTVM_RELEASE_CREATE_RESULT"], !resultPath.isEmpty else {
            return nil
        }
...
struct PreinstalledImageInstallConfiguration {
    ...
    static var current: PreinstalledImageInstallConfiguration? {
        ...
        guard let marker = arguments.firstIndex(of: "--riftvm-install-preinstalled-image"), marker + 1 < arguments.count,
...
struct PreinstalledImageManifest: Decodable {
    ...
    let minimumRiftVMVersion: String
    let product: Product
    let disk: Disk
    ...
    static func load(from url: URL) throws -> Self {
        ...
        guard runningVersion.compare(value.minimumRiftVMVersion, options: .numeric) != .orderedAscending else {
            throw ManifestError.version(required: value.minimumRiftVMVersion, actual: runningVersion)
        }
        return value
    }
    ...
    enum ManifestError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .version(let required, let actual): "This image requires RiftVM \(required) or newer; this app is \(actual)."
            }
        }
    }
}
...
struct HeadlessLaunchConfiguration {
    ...
    static var current: HeadlessLaunchConfiguration? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let marker = arguments.firstIndex(of: "--riftvm-headless"), marker + 1 < arguments.count,
        ...
        return HeadlessLaunchConfiguration(
            ...
            showsWindow: arguments.contains("--riftvm-show-window")
        )
    }
}
