...
struct VMOSMainVirtualMachineView: View {
    ...
    var body: some View {
                ...
                VStack(spacing: 14) {
                    ...
                    HStack {
                        ...
                        Button("Export Diagnostics", systemImage: "square.and.arrow.up") {
                            do {
                                _ = try RiftVMDiagnostics.export()
                            } catch let error as CocoaError where error.code == .userCancelled {
                                // The save panel was intentionally dismissed.
                            } catch {
                                RiftVMLog.error("Diagnostic export failed: \(error.localizedDescription)")
                            }
                        }
                    }
                }
        ...
        .alert(
            ...
            if runtimeState.canPersistMachineState {
                Text("RiftVM will save the virtual machine’s current state, stop it, and then close this window. You can resume from the same state next time.")
            } else if runtimeState.hasAttachedUSBAccessories {
                Text("A USB accessory is connected. RiftVM will shut down the guest instead of saving machine state so the accessory is released safely.")
            } else {
                Text("\(runtimeState.machineStateUnavailabilityReason ?? "Machine state is unavailable for this configuration.") RiftVM will ask the guest to shut down, force stop only if it does not respond, and then close this window.")
            }
        }
        ...
        .alert(
            ...
            Text("Confirm that you can sign in as “\(provisioningConfirmationUsername ?? "the new account")”. RiftVM will permanently remove the temporary provisioning password from this Mac’s Keychain. This cannot be undone.")
        }
        .alert("Use macOS Setup Assistant Instead?", isPresented: $isShowingManualSetupConfirmation) {
            ...
            Text("RiftVM will permanently remove the pending password from this Mac’s Keychain and will not submit automatic account setup again. Continue in Setup Assistant if the VM is running; otherwise close this window and run the VM again.")
        }
        .sheet(item: $settingsModel) { model in
            VMEditConfigurationView(model: model, appliesSharedFoldersImmediately: true)
        }
        .onReceive(NotificationCenter.default.publisher(for: .riftvmConfigurationSaved)) { notification in
    ...
    private var macGuestProvisioningOverlay: some View {
        ...
        case .needsVerification(let username):
            provisioningVerificationCard(
                ...
                detail: "The previous provisioning attempt was interrupted. Sign in as “\(username)” if the account exists; otherwise choose Retry Next Start. RiftVM will not submit it again automatically."
            )
    ...
    private func openSSH(address: String) {
        ...
        alert.informativeText = "Enter the Linux username for \(address). RiftVM does not store the username or any SSH credential."
    ...
    private func openSettings() {
        guard case let .success(model) = VMModel.loadConfigFromFile(rootPath: rootPath) else {
            sharedFolderResult = "RiftVM could not load this virtual machine’s settings."
            isShowingSharedFolderResult = true
            return
        }
        settingsModel = model
    ...
    private func addSharedFolder() {
        MacKitUtil.selectDirectory(title: "Choose a Folder to Share") { url in
            ...
            case .failure(let error):
                sharedFolderResult = "RiftVM could not add the shared folder: \(error)"
            }
            isShowingSharedFolderResult = true
        }
    }
    ...
    private func addDroppedSharedFolders(_ urls: [URL]) -> Bool {
        ...
        case .success:
            ...
            return true
        case .failure(let error):
            sharedFolderResult = "RiftVM could not add the shared folders: \(error)"
            isShowingSharedFolderResult = true
            return false
        }
    }
    ...
    private func chooseThumbnailImage() {
        ...
        guard let cgImage = thumbnail.cgImage(forProposedRect: nil, context: nil, hints: nil),
            ...
            MacKitUtil.alertWarn(title: "Invalid thumbnail", message: "RiftVM could not decode this image.")
            return
        }
        do {
