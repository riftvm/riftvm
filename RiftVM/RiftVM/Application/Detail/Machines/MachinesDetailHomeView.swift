...
struct MachinesDetailHomeView: View {
    ...
    func convertDiskToASIF(item: HomeItemVMModel, model: VMModel) {
        MacKitUtil.alertWarn(
            ...
            message: "RiftVM will create a space-efficient ASIF disk and keep the original raw disk as a backup. The machine must remain stopped during conversion."
        ) { isOK in
        ...
        MacKitUtil.alertWarn(
            ...
            case .success(let enrollment):
                do {
                    ...
                    panel.nameFieldStringValue = "RiftVM-Agent-Enrollment-\(model.config.name).json"
                    panel.canCreateDirectories = true
                    ...
                    MacKitUtil.alertInfo(
                        ...
                        message: "Install this file with the RiftVM Guest Agent inside “\(model.config.name)”, then delete the exported copy. The host token remains protected in RiftVM's private application data."
                    )
                } catch {
    ...
    func importMachine() {
        ...
        openPanel.title = "Select an RiftVM Export"
                ...
                .toolbar(id: "toolbar") {
                    ...
                    ToolbarItem(id: "import", placement: .primaryAction) {
                        Button(action: importMachine) {
                            Label("Import an RiftVM export", systemImage: "square.and.arrow.down")
                        }
                    }
