...
final class VMSnapshotManagerTests: XCTestCase {
    private static let realLowSpaceRootEnvironmentKey = "RIFTVM_REAL_LOW_SPACE_VOLUME"
    private var temporaryRoot: URL!

    override func setUpWithError() throws {
        temporaryRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("RiftVMTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRealNearlyFullVolumeRejectsSnapshotBeforeMutation() throws {
        ...
        let machine = volume.appendingPathComponent("NearFull.riftvm", isDirectory: true)
    ...
    func testLayeredRestoreSurvivesProcessTerminationAtEveryCheckpoint() throws {
        for checkpoint in VMSnapshotRestoreCheckpoint.allCases {
            ...
            environment["RIFTVM_SNAPSHOT_CRASH_ROOT"] = root.path
            environment["RIFTVM_SNAPSHOT_CRASH_ID"] = fixture.target.id
            environment["RIFTVM_SNAPSHOT_CRASH_CHECKPOINT"] = checkpoint.rawValue
    ...
    func testLayeredRestoreCrashChild() throws {
        ...
        guard let rootPath = environment["RIFTVM_SNAPSHOT_CRASH_ROOT"],
              let snapshotID = environment["RIFTVM_SNAPSHOT_CRASH_ID"],
              let checkpointName = environment["RIFTVM_SNAPSHOT_CRASH_CHECKPOINT"] else {
            return
        }
    ...
    func testKernelLockRejectsMutationOwnedByAnotherProcessContext() throws {
        ...
        XCTAssertTrue(message.contains("Another RiftVM process"), message)
        XCTAssertTrue(VMSnapshotManager.listSnapshots(vmRootPath: temporaryRoot).isEmpty)
    }
